<?php
    ...
    $query = "UPDATE equipos SET nombre = ?, anillos = ?, fundado = ? = NOW() WHERE id = ?";
    ...
    $st->bind_param("sisi", $nombres, $anillos, $fundado, $id);

    if($st->execute()){
        if($st->affected_rows > 0){
            echo json_encode(["message" => "Equipo actualizando correctamente"]);
        }else{
